...
test("Yaniv maps its five interactive cards directly to 1–5", () => {
  assert.deepEqual(
    Array.from({ length: 5 }, (_, index) => key({ owner: "player", index, gameType: "yaniv" })),
    ["1", "2", "3", "4", "5"],
  );
});
...
test("piles and contextual actions have mnemonic shortcuts", () => {
  ...
  assert.equal(key({ label: "call yaniv" }), "y");
  ...
  assert.equal(key({ label: "play cards" }), "enter");
  assert.equal(key({ label: "next round" }), "enter");
});
