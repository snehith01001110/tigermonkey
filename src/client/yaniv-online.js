...
import { createYanivTable } from "./yaniv-ui.js?v=mobile-scopa-rail";
...
export function startYanivOnlineGame(code) {
  roomCode = code;
  ui = createYanivTable({ dispatch: sendAction, online: true });
  ...
  else setupLobby({ roomCode, gameType: "yaniv", autoOpen: true, attachButton: false, onJoined: joined });
...
function connect() {
  ...
  if (!apiBase) {
    ...
    render();
    setupLobby({ roomCode, gameType: "yaniv", autoOpen: true, attachButton: false, onJoined: joined });
    return;
  }
  ...
  socket.addEventListener("message", (event) => {
    try {
      ...
      if (payload.type === "state") {
        if (payload.state.gameType !== "yaniv") {
